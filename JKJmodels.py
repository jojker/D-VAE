...
'''
    DAG Variational Autoencoder (D-VAE). (FOR ENAS ONLY)
    
    Change the one-hot type encoding to use a scaled value, then that scaled value is the seed I need
    Now I just to make sure that the decoder never gives a shape node more than one property value
    
    This is a flat architecture with shape nodes allowed to have parents and children
...
class DVAE_3D(nn.Module):
    def __init__(self, max_n, nvt, START_TYPE, END_TYPE, hs=501, nz=56, bidirectional=False, vid=True):
        ...
        self.pick_seed = nn.Sequential(
                ...
                nn.Linear(hs * 2, 1),
                nn.Sigmoid()
                )  # what seed value the vertex should have f(typescvore, hg)
    ...
    def _one_hot(self, idx, seed, length):
        if type(idx) in [list, range]:
            ...
            idx = torch.LongTensor([float(idx)]).unsqueeze(0)
            seed = torch.Tensor([float(seed)]).unsqueeze(0)
    ...
    def _propagate_to(self, G, v, propagator, H=None, reverse=False):
        ...
        seeds = [g.vs[v]['seeds'] for g in G]
        X = self._one_hot(v_types,seeds, self.nvt)
        if reverse:
    ...
    def decode(self, z, stochastic=True):
        ...
        for g in G:
            g.add_vertex(type=self.START_TYPE,seed=1.)
        ...
        for idx in range(1, self.max_n):
            ...
            if idx == self.max_n - 1:  # force the last node to be end_type
                ...
                seeds = self.pick_seed(Hg) 
                seeds=seeds.flatten().tolist()
            
            for i, g in enumerate(G):
                if not finished[i]:
                    g.add_vertex(type=new_types[i],seed=seeds[i])
    ...
    def loss(self, mu, logvar, G_true, beta=0.005):
        ...
        for v_true in range(1, self.max_n):
            ...
            true_seeds = [g_true.vs[v_true]['seed'] if v_true < g_true.vcount() 
            ...
            for i, g in enumerate(G):
                if true_types[i] != self.START_TYPE:
                    g.add_vertex(type=true_types[i], seed=true_seeds[i])
            self._update_v(G, v_true)
            
            # seed log differences 
            seeds2remove=true_seeds.index(0.)
            true_seeds.remove(seeds2remove)
            pred_seeds = self.pick_seed(Hg).flatten().tolist().remove(seeds2remove)
            sll = - F.binary_cross_entropy(torch.Tensor(pred_seeds), torch.Tensor(true_seeds), reduction='sum')*len(G_true)/len(pred_seeds) 
            res = res + sll
            #logDiffs=[np.log(1-abs(t-p)) if t>0 else 0 for t,p in zip(true_seeds,pred_seeds)]
            #meanLogDiff=sum(logDiffs)/sum(t > 0 for t in true_seeds)
            #res = res + meanLogDiff
